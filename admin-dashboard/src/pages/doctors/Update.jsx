  ...
  const onSubmit = (data) => {
    ...
    if (category === "patients") {
      updatedObject = {
        ...
        cv: data.cv ? data.cv[0]?.name : object.cv, // Assuming cv is a file input
        image: data.image ? data.image[0]?.name : object.image, // Assuming image is a file input
      };
    } else if (category !== "departements") {
      updatedObject = {
        id: data.id,
    ...
    if (category === "doctors") {
      ...
      const updatedDoctorsData = doctorsData.map((doc) => {
        return String(doc.id) === String(personneId) ? updatedObject : doc;
      });
      ...
      const updatedInfermierData = infermiersData.map((inf) => {
        return String(inf.id) === String(personneId) ? updatedObject : inf;
      });
      ...
      const updatedPatientData = patientsData.map((pat) => {
        return String(pat.id) === String(personneId) ? updatedObject : pat;
      });
      ...
      const updatedDepartementData = departementsData.map((dep) => {
        return String(dep.id) === String(personneId) ? updatedObject : dep;
      });
